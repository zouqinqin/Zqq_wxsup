...
//		ErrorController
	...
	@ResponseBody
	public ResponseResult login(UserDTO user, @RequestParam(value = "rememberMe", required = false) boolean rememberMe, HttpSession session) {
...
//		User existUser = this.userService.findUserByMobile(user.getMobile());
		User existUser = this.userService.findUser(user.getUsername(),user.getPassword());
		if (existUser == null) {
			...
			//add 2018/08/31 chenjf start
			//缓存用户信息
			String userId = existUser.getId().toString();
			UserRoleDTO userRoleDTO = userService.getUserInfo(userId);
			session.setAttribute("userInfo",userRoleDTO);
			//add 2018/08/31 chenjf end
		}
		// 用户登录
		try {
